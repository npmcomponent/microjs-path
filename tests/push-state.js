...
function update(token) {
  actualTotal += token + '::';
  if (!window.mochaPhantomJS) console.log(token);
  if (!expected.length) {
...
function define(route, tokens, description) {
  describe(route, function () {
    it(description, function (done) {
      ...
      if (route === 'root') Path.history.listen();
      else if (route === 'back') history.go(-1);
      else Path.history.pushState({}, '', route);
    });
  });
}
...
(function () {
  ...
  describe('end', function () {
    it('makes the following equivalent', function () {
